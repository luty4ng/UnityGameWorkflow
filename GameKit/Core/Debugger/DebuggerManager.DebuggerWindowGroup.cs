...
namespace GameKit.Debugger
{
    internal sealed partial class DebuggerManager : GameKitModule, IDebuggerManager
    {
        /// <summary>
        /// 调试器窗口组。
        /// </summary>
        private sealed class DebuggerWindowGroup : IDebuggerWindowGroup
        {
            ...
            public void RegisterDebuggerWindow(string path, IDebuggerWindow debuggerWindow)
            {
                if (string.IsNullOrEmpty(path))
                {
                    throw new GameKitException("Path is invalid.");
                }
                ...
                if (pos < 0 || pos >= path.Length - 1)
                {
                    ...
                    {
                        throw new GameKitException("Debugger window has been registered.");
                    }
                ...
                else
                {
                    ...
                    if (debuggerWindowGroup == null)
                    {
                        ...
                        {
                            throw new GameKitException("Debugger window has been registered, can not create debugger window group.");
                        }
