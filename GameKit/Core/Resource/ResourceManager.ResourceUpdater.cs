...
namespace GameKit.Resource
{
    internal sealed partial class ResourceManager : GameKitModule, IResourceManager
    {
        ...
        private sealed partial class ResourceUpdater
        {
            ...
            public GameKitAction<string, int, long> ResourceApplyStart;
            public GameKitAction<ResourceName, string, string, int, int> ResourceApplySuccess;
            public GameKitAction<ResourceName, string, string> ResourceApplyFailure;
            public GameKitAction<string, bool> ResourceApplyComplete;
            public GameKitAction<ResourceName, string, string, int, int, int> ResourceUpdateStart;
            public GameKitAction<ResourceName, string, string, int, int> ResourceUpdateChanged;
            public GameKitAction<ResourceName, string, string, int, int> ResourceUpdateSuccess;
            public GameKitAction<ResourceName, string, int, int, string> ResourceUpdateFailure;
            public GameKitAction<ResourceGroup, bool> ResourceUpdateComplete;
            public GameKitAction ResourceUpdateAllComplete;
            ...
            public void ApplyResources(string resourcePackPath)
            {
                ...
                {
                    throw new GameKitException("You must check resources complete first.");
                }

                if (m_ApplyingResourcePackStream != null)
                {
                    throw new GameKitException(Utility.Text.Format("There is already a resource pack '{0}' being applied.", m_ApplyingResourcePackPath));
                }

                if (m_UpdatingResourceGroup != null)
                {
                    throw new GameKitException(Utility.Text.Format("There is already a resource group '{0}' being updated.", m_UpdatingResourceGroup.Name));
                }

                if (m_UpdateWaitingInfoWhilePlaying.Count > 0)
                {
                    throw new GameKitException("There are already some resources being updated while playing.");
                }
                ...
                try
                {
                    ...
                    {
                        throw new GameKitException("Deserialize resource pack version list failure.");
                    }

                    if (versionList.Offset + versionList.Length != length)
                    {
                        throw new GameKitException("Resource pack length is invalid.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameKitException(Utility.Text.Format("Apply resources '{0}' with exception '{1}'.", resourcePackPath, exception), exception);
                }
            }
            ...
            public void UpdateResources(ResourceGroup resourceGroup)
            {
                if (m_DownloadManager == null)
                {
                    throw new GameKitException("You must set download manager first.");
                }
            ...
            public void StopUpdateResources()
            {
                ...
                if (m_UpdatingResourceGroup == null)
                {
                    throw new GameKitException("There is no resource group being updated.");
                }
            ...
            private void GenerateReadWriteVersionList()
            {
                ...
                try
                {
                    ...
                    {
                        throw new GameKitException("Serialize read-write version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameKitException(Utility.Text.Format("Generate read-write version list exception '{0}'.", exception), exception);
                }
            ...
            private void OnDownloadStart(object sender, DownloadStartEventArgs e)
            {
                ...
                {
                    throw new GameKitException(Utility.Text.Format("File '{0}' is too large.", e.DownloadPath));
                }
