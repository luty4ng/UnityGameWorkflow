...
namespace GameKit.FileSystem
{
    ...
    internal sealed partial class FileSystem : IFileSystem
    {
        ...
        private readonly GameKitMultiDictionary<int, int> m_FreeBlockIndexes;
        ...
        private FileSystem(string fullPath, FileSystemAccess access, FileSystemStream stream)
        {
            ...
            {
                throw new GameKitException("Full path is invalid.");
            }

            if (access == FileSystemAccess.Unspecified)
            {
                throw new GameKitException("Access is invalid.");
            }
            ...
            m_FreeBlockIndexes = new GameKitMultiDictionary<int, int>();
        ...
        public static FileSystem Create(string fullPath, FileSystemAccess access, FileSystemStream stream, int maxFileCount, int maxBlockCount)
        {
            if (maxFileCount <= 0)
            {
                throw new GameKitException("Max file count is invalid.");
            }

            if (maxBlockCount <= 0)
            {
                throw new GameKitException("Max block count is invalid.");
            }

            if (maxFileCount > maxBlockCount)
            {
                throw new GameKitException("Max file count can not larger than max block count.");
            }
        ...
        public byte[] ReadFile(string name)
        {
            ...
            {
                throw new GameKitException("File system is not readable.");
            }
        ...
        public int ReadFile(string name, Stream stream)
        {
            ...
            if (!stream.CanWrite)
            {
                throw new GameKitException("Stream is not writable.");
            }
        ...
        public byte[] ReadFileSegment(string name, int offset, int length)
        {
            ...
            if (offset < 0)
            {
                throw new GameKitException("Index is invalid.");
            }
        ...
        public bool WriteFile(string name, byte[] buffer, int startIndex, int length)
        {
            ...
            {
                throw new GameKitException("File system is not writable.");
            }
            ...
            {
                throw new GameKitException(Utility.Text.Format("Name '{0}' is too long.", name));
            }
        ...
        public bool WriteFile(string name, Stream stream)
        {
            ...
            if (!stream.CanRead)
            {
                throw new GameKitException("Stream is not readable.");
            }
        ...
        public bool WriteFile(string name, string filePath)
        {
            ...
            {
                throw new GameKitException("File path is invalid");
            }
        ...
        public bool RenameFile(string oldName, string newName)
        {
            ...
            {
                throw new GameKitException("Old name is invalid.");
            }

            if (string.IsNullOrEmpty(newName))
            {
                throw new GameKitException("New name is invalid.");
            }

            if (newName.Length > byte.MaxValue)
            {
                throw new GameKitException(Utility.Text.Format("New name '{0}' is too long.", newName));
            }
        ...
        private bool TryCombineFreeBlocks(int freeBlockIndex)
        {
            ...
            foreach (KeyValuePair<int, GameKitLinkedListRange<int>> blockIndexes in m_FreeBlockIndexes)
            {
        ...
        {
            GameKitLinkedListRange<int> lengthRange = default(GameKitLinkedListRange<int>);
        ...
        private int AllocBlock(int length)
        {
            ...
            int lengthFound = -1;
            GameKitLinkedListRange<int> lengthRange = default(GameKitLinkedListRange<int>);
            foreach (KeyValuePair<int, GameKitLinkedListRange<int>> i in m_FreeBlockIndexes)
            {
                if (i.Key < length)
                {
                    continue;
