...
namespace GameKit.Download
{
    ...
    internal sealed partial class DownloadManager : GameKitModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, string tag, int priority, object userData)
        {
            ...
            {
                throw new GameKitException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new GameKitException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new GameKitException("You must add download agent first.");
            }
