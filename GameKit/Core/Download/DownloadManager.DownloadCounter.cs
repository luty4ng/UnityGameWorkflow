namespace GameKit.Download
{
    internal sealed partial class DownloadManager : GameKitModule, IDownloadManager
    {
        ...
        {
            private readonly GameKitLinkedList<DownloadCounterNode> m_DownloadCounterNodes;
            ...
            public DownloadCounter(float updateInterval, float recordInterval)
            {
                ...
                {
                    throw new GameKitException("Update interval is invalid.");
                }

                if (recordInterval <= 0f)
                {
                    throw new GameKitException("Record interval is invalid.");
                }

                m_DownloadCounterNodes = new GameKitLinkedList<DownloadCounterNode>();
