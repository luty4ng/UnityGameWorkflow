...
namespace GameKit.Network
{
    ...
    internal sealed partial class NetworkManager : GameKitModule, INetworkManager
    {
        ...
        public INetworkChannel CreateNetworkChannel(string name, ServiceType serviceType, INetworkChannelHelper networkChannelHelper)
        {
            ...
            {
                throw new GameKitException("Network channel helper is invalid.");
            }

            if (networkChannelHelper.PacketHeaderLength < 0)
            {
                throw new GameKitException("Packet header length is invalid.");
            }

            if (HasNetworkChannel(name))
            {
                throw new GameKitException(Utility.Text.Format("Already exist network channel '{0}'.", name ?? string.Empty));
            }
            ...
            switch (serviceType)
            {
                ...
                default:
                    throw new GameKitException(Utility.Text.Format("Not supported service type '{0}'.", serviceType));
            }
