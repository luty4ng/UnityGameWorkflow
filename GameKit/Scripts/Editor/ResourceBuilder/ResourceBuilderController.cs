...
namespace UnityGameKit.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        public string GameKitVersion
        {
            get
            {
                return GameKit.Version.GameKitVersion;
            }
        }
        ...
        public event GameKitAction<int, int> OnLoadingResource = null;

        public event GameKitAction<int, int> OnLoadingAsset = null;

        public event GameKitAction OnLoadCompleted = null;

        public event GameKitAction<int, int> OnAnalyzingAsset = null;

        public event GameKitAction OnAnalyzeCompleted = null;

        public event GameKitFunc<string, float, bool> ProcessingAssetBundle = null;

        public event GameKitFunc<string, float, bool> ProcessingBinary = null;

        public event GameKitAction<Platform> ProcessResourceComplete = null;

        public event GameKitAction<string> BuildResourceError = null;
        ...
        public bool Load()
        {
            ...
            try
            {
                ...
                XmlNode xmlRoot = xmlDocument.SelectSingleNode("UnityGameKit");
        ...
        public bool Save()
        {
            try
            {
                ...
                XmlElement xmlRoot = xmlDocument.CreateElement("UnityGameKit");
        ...
        public bool BuildResources()
        {
            ...
            m_BuildReport.Initialize(BuildReportPath, ProductName, CompanyName, GameIdentifier, GameKitVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
            ...
            try
            {
                ...
                if (m_BuildEventHandler != null)
                {
                    ...
                    m_BuildEventHandler.OnPreprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, GameKitVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                ...
                if (!m_ResourceCollection.Load())
                {
                    ...
                    if (m_BuildEventHandler != null)
                    {
                        ...
                        m_BuildEventHandler.OnPostprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, GameKitVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                            Platforms, AssetBundleCompression, CompressionHelperTypeName, AdditionalCompressionSelected, ForceRebuildAssetBundleSelected, BuildEventHandlerTypeName, OutputDirectory, buildAssetBundleOptions,
                            WorkingPath, OutputPackageSelected, OutputPackagePath, OutputFullSelected, OutputFullPath, OutputPackedSelected, OutputPackedPath, BuildReportPath);
                    }
        ...
        private void ProcessPackageVersionList(string outputPackagePath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(packageVersionListPath, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    throw new GameKitException("Serialize package version list failure.");
                }
            }
        }
        ...
        private VersionListData ProcessUpdatableVersionList(string outputFullPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(updatableVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new GameKitException("Serialize updatable version list failure.");
                }
            }
        ...
        private void ProcessReadOnlyVersionList(string outputPackedPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(readOnlyVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new GameKitException("Serialize read-only version list failure.");
                }
            }
        }
        ...
        private void CreateFileSystems(IEnumerable<ResourceData> resourceDatas, string outputPath, Dictionary<string, IFileSystem> outputFileSystem)
        {
            ...
            {
                m_FileSystemManager = GameKitModuleCenter.GetModule<IFileSystemManager>();
        ...
        private static BuildTarget GetBuildTarget(Platform platform)
        {
            switch (platform)
            {
                ...
                default:
                    throw new GameKitException("Platform is invalid.");
            }
        }
