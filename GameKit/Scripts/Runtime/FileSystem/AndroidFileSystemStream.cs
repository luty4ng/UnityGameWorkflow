...
namespace UnityGameKit.Runtime
{
    ...
    public sealed class AndroidFileSystemStream : FileSystemStream
    {
        ...
        static AndroidFileSystemStream()
        {
            ...
            if (unityPlayer == null)
            {
                throw new GameKitException("Unity player is invalid.");
            }

            AndroidJavaObject currentActivity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity");
            if (currentActivity == null)
            {
                throw new GameKitException("Current activity is invalid.");
            }

            AndroidJavaObject assetManager = currentActivity.Call<AndroidJavaObject>("getAssets");
            if (assetManager == null)
            {
                throw new GameKitException("Asset manager is invalid.");
            }
        ...
        public AndroidFileSystemStream(string fullPath, FileSystemAccess access, bool createNew)
        {
            ...
            {
                throw new GameKitException(Utility.Text.Format("'{0}' is not supported in AndroidFileSystemStream.", access));
            }

            if (createNew)
            {
                throw new GameKitException("Create new is not supported in AndroidFileSystemStream.");
            }

            int position = fullPath.LastIndexOf(SplitFlag, StringComparison.Ordinal);
            if (position < 0)
            {
                throw new GameKitException("Can not find split flag in full path.");
            }
            ...
            {
                throw new GameKitException(Utility.Text.Format("Open file '{0}' from Android asset manager failure.", fullPath));
            }
        ...
        protected override long Position
        {
            get
            {
                throw new GameKitException("Get position is not supported in AndroidFileSystemStream.");
            }
            set
            {
        ...
        {
            throw new GameKitException("SetLength is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new GameKitException("WriteByte is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new GameKitException("Write is not supported in AndroidFileSystemStream.");
        }
        ...
        {
            throw new GameKitException("Flush is not supported in AndroidFileSystemStream.");
        }
